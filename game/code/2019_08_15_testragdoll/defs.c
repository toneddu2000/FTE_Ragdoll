...
pr_dumpplatform -O temp5282 -Ffte -Tcs
Available options:
...
void(float entnum) CSQC_Ent_Spawn;	/* Clumsily defined function for compat with DP. Should call spawn, set that en's entnum field, and return the entity inside the 'self' global which will then be used for fllowing Ent_Updates. MUST NOT PARSE ANY NETWORK DATA (which makes it kinda useless). */
...
void(vector v1, vector v2, float flags, entity en) traceline = #16; /*
		Traces a thin line through the world from v1 towards v2.
		Will not collide with en, en.owner, or any entity who's owner field refers to en.
		...
		flags&MOVE_LAGGED will backdate entity positions for the purposes of this builtin according to the indicated player en's latency, to provide lag compensation. */
...
float(entity en) checkbottom = #40; /*
...
void(entity en) changepitch = #63; /* Part of DP_QC_CHANGEPITCH*/
void(entity en, entity ignore) tracetoss = #64;
string(entity en) etos = #65; /* Part of DP_QC_ETOS*/
...
void(vector start, vector mins, vector maxs, vector end, float nomonsters, entity en) tracebox = #90; /* Part of DP_QC_TRACEBOX
...
vector(entity en, float tagnum) rotatevectorsbytag = #244;
...
float(float skel, entity en, float modelindex, float retainfrac, float firstbone, float lastbone, optional float addfrac) skel_build = #264; /* Part of FTE_CSQC_SKELETONOBJECTS
...
void(optional entity en, optional vector neworigin) touchtriggers = #279; /*
...
void(entity en, float bonenum, vector org, optional vector angorfwd, optional vector right, optional vector up) skel_set_bone_world = #283; /*
...
void(float mask) addentities = #301; /*
		Walks through all entities effectively doing this:
		 if (en.drawmask&mask){ if (!en.predaw()) addentity(en); }
...
void(float effectnum, entity en, vector start, vector end) trailparticles = #336; /* Part of FTE_SV_POINTPARTICLES
		Draws the given effect between the two named points. If en is not world, distances will be cached in the entity in order to avoid framerate dependancies. The entity is not otherwise used. */
...
void(entity en) runstandardplayerphysics = #347; /*
...
int(float playernum, string keyname, optional void *outptr, int size) getplayerkeyblob = #0:getplayerkeyblob; /*
		Obtains a copy of the full data blob. Will write up to size bytes and return the actual size. Does not null terminate (but memalloc(ret+1) will, if you want to cast the buffer to a string), and the blob may contain embedded nulls. Ignores all special keys, returning only what is actually there. */
...
int(int buf, string key, optional void *ptr, int size) serverkeyblob = #0:serverkeyblob; /*
		Version of serverkey that can obtain entire serverinfo, localinfo, or (local)userinfo blobs. Returns blob size */
...
void(float skinobj) releasecustomskin = #379; /*
		Lets the engine know that the skin will no longer be needed. Thanks to refcounting any ents with the skin already applied will retain their skin until later changed. It is valid to destroy a skin just after applying it to an en in the same function that it was created in, as the skin will only be destroyed once its refcount rops to 0. */
...
float(entity en, string tagname) gettagindex = #451; /* Part of DP_MD3_TAGSINFO*/
vector(entity en, float tagindex) gettaginfo = #452; /* Part of DP_MD3_TAGSINFO
...
string(float fieldnum, entity en) getentityfieldstring = #499; /* Part of DP_QC_ENTITYDATA*/
float(float fieldnum, entity en, string s) putentityfieldstring = #500; /* Part of DP_QC_ENTITYDATA*/
...
float(entity en) num_for_edict = #512;
...
void(string s) loadfromdata = #529; /*
		Reads a set of entities from the given string. This string should have the same format as a .en file or a saved game. Entities will be spawned as required. If you need to see the entities that were created, you should use parseentitydata instead. */

void(string s) loadfromfile = #530; /*
		Reads a set of entities from the named file. This file should have the same format as a .en file or a saved game. Entities will be spawned as required. If you need to see the entities that were created, you should use parseentitydata instead. */
